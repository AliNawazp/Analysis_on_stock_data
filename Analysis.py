...
def show_data(option):
    ...
    if option == 'Sunsex':
        st.write('You selected:', option)
        st.subheader("The BSE stock data")
        col1, col2 = st.columns(2)
        with col1:
            st.write(df_sunsex)
        with col2:
            image = Image.open("BSE_image.jpg")
            st.image(image, caption='BSE_office')
        st.markdown("<h2 style='text-align: left; color: black;'>'Data Analysis on Sunsex'</h2>", unsafe_allow_html=True)
        Analysis_on_Sunsex()
